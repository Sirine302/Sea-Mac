...
HWND  g_hWnd;											// donn�es en rapport avec la fenetre
RECT  g_rRect;											// donn�es en rapport avec la fenetre
HDC   g_hDC;											
HGLRC g_hRC;											
...
// tableau qui va contenir les donn�ess de la height map (fichier .raw qui va stock� des valeurs entre 0 et 255.
...
void Init(HWND hWnd)
{
	g_hWnd = hWnd;										// donn�es en rapport avec la fenetre
	GetClientRect(g_hWnd, &g_rRect);					// donn�es en rapport avec la fenetre
	InitializeOpenGL(g_rRect.right, g_rRect.bottom);	// donn�es en rapport avec la fenetre

	
	// on lit le fichier height map (fichier .raw) et on met les donn�es dans le tab g_HeightMap
	LoadRawFile("Terrain.raw", MAP_SIZE * MAP_SIZE, g_HeightMap);	

	// on donne un pont de vision de d�part pour la camera
...
bool AnimateNextFrame(int desiredFrameRate)
{
	...
	// verification qu'on a les fps souhait�s
...
WPARAM MainLoop()
{
	...
	{													
		...
		{ 		
			if(AnimateNextFrame(60))					// on fait en sorte d'anim� 60 fps
			{
				g_Camera.Update();							// maj des donn�es de la camera
				RenderScene();							
			}
			else
			{
	...
	DeInit();											//on nettoie la memoire allou�e

	return(msg.wParam);									
}

// rendu de la sc�ne entiere
void RenderScene() 
{
